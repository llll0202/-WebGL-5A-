...
class EventTarget {
  captureHandlers: Record<
    string,
    Array<{ once: boolean; callback: (event?: any) => void }>
  >;
  bubbleHandlers: Record<
    string,
    Array<{ once: boolean; callback: (event?: any) => void }>
  >;
  __eventTimers__: any[];
...
export default class SphericalCamera extends EventTarget {
  ...
  dragMove(e: MouseEvent): void {
    ...
    if (e.buttons === 1) {
      ...
      this.lookAt(
        this.center.x - p[0] * radius,
        this.center.y,
        this.center.z - p[1] * radius,
      );
    } else {
      this.props._theta -=
        (2 * Math.PI * deltaX) / (this.domElement as HTMLElement).clientHeight;
      this.props._phi -=
        (2 * Math.PI * deltaY) / (this.domElement as HTMLElement).clientHeight;

      this.props._phi = Math.max(
        EPS,
  ...
  wheel(e: WheelEvent): void {
    ...
    //限制放大缩小的范围
    this.props._radius = Math.max(
      Math.min(this.props._radius, MAX_RADIUS),
      MIN_RADIUS,
    );
  ...
  //获取相机的视图矩阵，getMatrix方法会根据相机的半径、phi、theta和中心点来计算相机的视图矩阵，并返回这个矩阵
  getMatrix(): any {
