...
function pointInRect(rect: number[], point: number[]): boolean {
  return (
    inRange(rect[0], rect[0] + rect[2], point[0]) && inRange(rect[1], rect[1] + rect[3], point[1])
  )
}
...
function measureText(
  ...
  if (text instanceof Array) {
    for (const t of text) {
      ...
      const rect = (myCtx as any).measureText(t)
      width = Math.max(width, rect.width)
      height =
        rect.fontBoundingBoxAscent == null
          ? style.fontSize
          : rect.fontBoundingBoxAscent + rect.fontBoundingBoxDescent
    }
  } else {
    if (style) {
      ;(myCtx as any).font = (
    ...
    const rect = (myCtx as any).measureText(text)
    width = rect.width
    height =
      rect.fontBoundingBoxAscent == null
        ? style.fontSize
...
function collision(b1: number[], b2: number[]): boolean {
  const dx = Math.abs(b1[0] + b1[2] / 2 - b2[0] - b2[2] / 2)
  const dy = Math.abs(b1[1] + b1[3] / 2 - b2[1] - b2[3] / 2)
  const dw = (b1[2] + b2[2]) / 2
  const dh = (b1[3] + b2[3]) / 2
  return dx <= dw && dy <= dh
}
...
function deal(
  ...
  if (!floor) return num
  const value = Math.floor(num * pixelRatio) / pixelRatio
  if (!value && num) {
...
function compute(style: any, size: number, ratio: number = window.devicePixelRatio): number {
  ...
  const isAdd = style.indexOf('+') > 0
  const isReduce = style.indexOf('-') > 0
  if (isAdd || isReduce) {
    let arr: string[]
    if (isAdd) {
      arr = style.split('+')
    ...
    if (n1.endsWith('%') && n2.endsWith('px')) {
      const percent = (n1.substring(0, n1.length - 1) as any as number) - 0
      x1 = percent * size * 0.01
      x2 = (n2.substring(0, n2.length - 2) as any as number) - 0
    } else if (n1.endsWith('px') && n2.endsWith('%')) {
      const percent = (n2.substring(0, n2.length - 1) as any as number) - 0
    ...
    if (style.endsWith('%')) {
      const percent = style.substring(0, style.length - 1) - 0
...
function isCollisionArea(ps: number[][], p: number[]): boolean {
  ...
  const tp = [p[0] + Math.cos(d), p[1] + Math.sin(d)]
...
function isIntersection(p1: number[], p2: number[], p3: number[], p4: number[]): boolean | null {
  ...
  //k1,k2为两条线的斜率，如果斜率相同则两条线平行或者重合，不相交
  const k1 = dy1 / dx1
  const k3 = dy3 / dx3
  if (k1 == k3) return null
  //n为交点坐标在p1p2线段上的位置,如果n在0-1之间则交点在线段上，
  // 如果n<0则交点在线段外且在p1的反方向，如果n>1则交点在线段外且在p2的反方向
  const n = (p3[1] + k3 * (p1[0] - p3[0]) - p1[1]) / (dy1 - k3 * dx1)
  if (n < 0 || n > 1) {
    return false
  }
  //m为交点坐标在p3p4线段上的位置,如果m在0-1之间则交点在线段上，
  // 如果m<0则交点在线段外且在p3的反方向，如果m>1则交点在线段外,指向tp的方向
  const m = (p1[0] + dx1 * n - p3[0]) / dx3
  if (isNaN(m)) {
    return false
  }
  if (m <= 0) {
...
function bezier(
  ...
  const x =
    startPoint[0] * Math.pow(1 - t, 3) +
    3 * control1[0] * t * Math.pow(1 - t, 2) +
    3 * control2[0] * Math.pow(t, 2) * (1 - t) +
    endPoint[0] * Math.pow(t, 3)
  const y =
    startPoint[1] * Math.pow(1 - t, 3) +
    3 * control1[1] * t * Math.pow(1 - t, 2) +
    3 * control2[1] * Math.pow(t, 2) * (1 - t) +
...
function quadratic(
  ...
  const t2 = t * t
  const x = startPoint[0] * Math.pow(1 - t, 2) + 2 * t * (1 - t) * control[0] + t2 * endPoint[0]
  const y = startPoint[1] * Math.pow(1 - t, 2) + 2 * t * (1 - t) * control[1] + t2 * endPoint[1]
  return [x, y]
}

function quadratic3d(
  startPoint: number[],
  control: number[],
  endPoint: number[],
  t: number,
  ...
  const x = startPoint[0] * Math.pow(1 - t, 2) + 2 * t * (1 - t) * control[0] + t2 * endPoint[0]
  const y = startPoint[1] * Math.pow(1 - t, 2) + 2 * t * (1 - t) * control[1] + t2 * endPoint[1]
  const z = startPoint[2] * Math.pow(1 - t, 2) + 2 * t * (1 - t) * control[2] + t2 * endPoint[2]
  return [x, y, z]
}
...
function interceptionQuadratic(
  startPoint: number[],
  control: number[],
  endPoint: number[],
  s: number,
  ...
  if (s === e) return null
  const e1 = mixPoint(startPoint, control, e)
  const e2 = mixPoint(control, endPoint, e)
  const ctrl_n = mixPoint(e1, e2, s)
  const s_n = quadratic(startPoint, control, endPoint, s)
  const e_n = quadratic(startPoint, control, endPoint, e)
  return [s_n, ctrl_n, e_n]
}

function interceptionBezier(
  startPoint: number[],
  control1: number[],
  control2: number[],
  endPoint: number[],
  ...
  if (s === e) return null
  const s1 = mixPoint(startPoint, control1, s)
  const s2 = mixPoint(control1, control2, s)
  const s3 = mixPoint(control2, endPoint, s)
  const e1 = mixPoint(startPoint, control1, e)
  const e2 = mixPoint(control1, control2, e)
  const e3 = mixPoint(control2, endPoint, e)
  const ss1 = mixPoint(s1, s2, s)
  const ss2 = mixPoint(s2, s3, s)
  const ee1 = mixPoint(e1, e2, e)
  const ee2 = mixPoint(e2, e3, e)
  const ctrl_start = mixPoint(ss1, ss2, e)
  const ctrl_end = mixPoint(ee2, ee1, 1 - s)
  const s_n = bezier(startPoint, control1, control2, endPoint, s)
  const e_n = bezier(startPoint, control1, control2, endPoint, e)
  return [s_n, ctrl_start, ctrl_end, e_n]
}

function fontSize(style: string): number {
  const pixel = style.match(/\d+px/)?.[0]
  if (!pixel) return 0
...
function pointToLineDistance(p1: number[], p2: number[], point: number[]): number {
  const v13 = [point[0] - p1[0], point[1] - p1[1]]
  ...
  const v23 = [point[0] - p2[0], point[1] - p2[1]]

  if (dot(v21, v23) < 0) return length(v23)

  const v31 = [-v13[0], -v13[1]]

  const normal = [-v12[1], v12[0]]
  const l = length(v31)
  const cosT = dot(normal, v31) / (l * length(normal))
...
function roundBoxDistance(
  ...
  width /= 2
  height /= 2
  const dx = Math.abs(x) - width + (radius as number),
    dy = Math.abs(y) - height + (radius as number)
  return (
    length([max(dx, 0.0) as number, max(dy, 0.0) as number]) +
    (min(max(dx, dy), 0.0) as number) -
    (radius as number)
  )
}
